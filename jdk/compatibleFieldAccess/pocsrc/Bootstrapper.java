...
public class Bootstrapper {
	public static CallSite getFunction(Lookup lookup, String name,
		...
		try {
			...
			return new ConstantCallSite(MethodHandles.throwException(
			...
			try {
				...
				for (Method method : methods) {
					if (Modifier.isStatic(method.getModifiers()) == staticProperty
						...
						Accessor annotation = method
								.getAnnotation(Accessor.class);
						if (annotation != null
								&& annotation.value().equals(name)) {
							MethodType mt = MethodType.methodType(void.class,
									method.getParameterTypes());
							MethodHandle ret = staticProperty ? lookup
									.findStatic(clazz, method.getName(), mt)
